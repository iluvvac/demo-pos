...
export default function OrdersPage() {
  ...
  const { data: recent } = useSWR<{ queues: { queue_number: string; order_code: string }[] }>(
    "/api/queue/list",
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex items-center gap-3">
            <Button
              variant="secondary"
              onClick={async () => {
                const res = await postJSON("/api/orders/calc-price", { jenis, ukuran, jumlah, bahan, finishing })
                setPrice(res.total)
              }}
            >
              Hitung Harga
          ...
          {created && (
            <div className="text-sm text-muted-foreground">
              Order dibuat: {created.order_code} – Status: {created.status_readable}
            </div>
          )}
        </CardContent>
      </Card>
